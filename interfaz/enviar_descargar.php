<?php
	...
	if(isset($_SESSION["identificador"]) && $_SESSION["identificador"] == session_id() && $_SESSION["id_tipo"] == 2){
		if((time() - $_SESSION["tiempo_inicio"]) > 7200){
			...
			if(isset($_GET["dr"])){
				$numeroReporte = substr($_GET["dr"], -1);
				$fecha = date("Y-m-d");
				...
				//Movimiento
				$titulo = "DESCARGAR REPORTE DIARIO";
				$descripcion = "Usuario ha descargado el reporte diario N&deg;" . $numeroReporte . " el " . $nuevaConexion->enviarFechaHora();
							...
							<table class="table table-striped mt-3">
								...
								<?php
									if(file_exists("../archivos/" . $_SESSION["id_usuario"] . "/")){
										$archivos = scandir("../archivos/" . $_SESSION["id_usuario"] . "/");
										$num = 0;
										for($i=2;$i<count($archivos);$i++){
											$arreglo = explode("N", $archivos[$i]);
								...
								?>
										<tr>
			      							...
			      							<td>
			      								<?php
			      									$arrayNombre = explode("N", $archivos[$i]);
			      									$arrayNombre_dos = explode(".", $arrayNombre[1]);		      									
			      								?>
			      								<a title="Descargar Archivo" href="../archivos/<?php echo $_SESSION['id_usuario']; ?>/<?php echo $archivos[$i]; ?>" download="<?php echo $archivos[$i]; ?>" style="color: #224875; font-size:18px;" onclick="location.href='enviar_descargar.php?dr=2001<?php echo $arrayNombre_dos[0]; ?>'"> 
			      									<i class="fa-solid fa-download"></i></span>
			      								</a>
			      							</td>      
			      							<td>
			      								<a title="Enviar Archivo" href="enviar.php?val=<?php echo $arreglo2[0]; ?>" style="color: #224875; font-size:18px;">
