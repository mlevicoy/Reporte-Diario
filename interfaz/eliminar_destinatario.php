			...
			if($resultado){
				if($nuevaConexion->obtenerFilasAfectadas() > 0){
					while($filas = $nuevaConexion->obtenerFilas($resultado)){
						$nombre = ucfirst(strtolower($filas[1])) . " " . ucfirst(strtolower($filas[2])) . " - " . $filas[3];
						$arreglo[] = $nombre;
						$idDestinatario[] = $filas[0];
					}
				}
			}			
?>
			...
			<body class="d-flex flex-column min-vh-100">
				...
				<main>
					<section>
						<div class="container">
							<form action="../comunicacion/quitar_destinatario.php" method="POST">
								<?php
								...
								<div class=" row justify-content-md-center mt-3 mb-3">
									<div class="col-md-4 col-lg-4 col-6">
										<label for=""><b>Eliminar Destinatario</b>:</label>
									</div>
								...
								<div class=" row justify-content-md-center mt-3 mb-3">
									<div class="col-md-4 col-lg-4 col-6">
										<select class="form-select" aria-label="Default select example" name="destinatario" required>
											<option value="" selected>Seleccionar destinatario</option>
											<?php
												for($i=0;$i<count($arreglo);$i++){
													echo "<option value=" . $idDestinatario[$i] . ">" . $arreglo[$i] . "</option>";
												}
											?>
										</select>
									</div>
									<div class="col-md-4 col-lg-4 d-grid col-6">
										<button class="btn btn-primary" type="submit" style="background-color: #224875;">Eliminar</button>
									</div>
								</div>
							</form>
						</div>	
