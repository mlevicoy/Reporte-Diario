<?php
	...
	if(isset($_SESSION["identificador"]) && $_SESSION["identificador"] == session_id() && $_SESSION["id_tipo"] == 2){
		if((time() - $_SESSION["tiempo_inicio"]) > 7200){
			...
			$query = "UPDATE proyecto SET nombre_proyecto = '" . $nombre_proyecto . "', numero_proyecto = '" . $numero_proyecto . "', monto_proyecto = '" . $monto_proyecto . "', mandante_proyecto = '" . $mandante_proyecto . "', contratista_proyecto = '" . $contratista_proyecto . "', inspecto_proyecto = '" . $inspector_proyecto . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);

			//Actualizamos la info de reporte
			$query = "UPDATE reporte SET fecha_reporte = '" . $fecha_reporte . "', revision_reporte = '". $revision_reporte ."' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
			
			//Actualizar la info de trabajo
			$query = "UPDATE trabajo SET sector_trabajo = '" . $sector_trabajo . "', lugar_trabajo = '" . $lugar_trabajo . "', turnomanana_trabajo = '" . $turnomanana_trabajo. "', turnotarde_trabajo = '" . $turnotarde_trabajo . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"]; 
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
			
			//Actualizar la info de plazo_trabajo
			$query = "UPDATE plazo_trabajo SET fechainicio_plazo = '" . $fechainicio_plazo . "', fechatermino_plazo = '" . $fechatermino_plazo . "', plazo_plazo = " . $plazo . ", entregaterreno_plazo = '" . $entregaterreno_plazo . "', diasrestantes_plazo = " . $diasrestantes_plazo . " WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"]; 
			$resultado	= $nuevaConexion->ejecutarConsulta($query);

			//Actualizar la info de horas
			$query = "UPDATE horas SET inicionormal_horas = '" . $inicionormal_horas . "', inicioextra_horas = '" . $inicioextra_horas . "', terminonormal_horas = '" . $terminonormal_horas . "', terminoextra_horas = '" . $terminoextra_horas . "', informenormal_horas = '" . $informenormal_horas . "', informeextra_horas = '" . $informeextra_horas . "', totalnormal_horas = '" . $totalnormal_horas . "', totalextra_hora = '" . $totalextra_horas . "', trayectonormal_horas = '" . $trayectonormal_horas . "', trayectoextra_horas = '" . $trayectoextra_horas . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
			
			//Actualizar la info de personaleecc
			$query = "UPDATE personaleecc SET administrador_si = '" . $administrador_si . "', administrador_no = '" . $administrador_no . "', terreno_si = '" . $terreno_si . "', terreno_no = '" . $terreno_no . "', calidad_si = '" . $calidad_si . "', calidad_no = '" . $calidad_no . "', oocc_si = '" . $oocc_si . "', oocc_no = '" . $oocc_no . "', prevencionista_si = '" . $prevencionista_si . "', prevencionista_no = '" . $prevencionista_no . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);

			//Actualizar la info de equipoeecc
			$query = "UPDATE equipoeecc SET comentario_equipoeecc = '" . $comentario_equipoeecc . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);

			//Actualizar la info de trabajadores
			$query = "UPDATE trabajadores SET fechainicio_trabajadores = '" . $fechainicio_trabajadores . "', fechatermino_trabajadores = '" . $fechatermino_trabajadores . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"]; 
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
						
			//Actualizar la info de condición
			$query = "UPDATE condicion SET tiene_c_condicion = '" . $tiene_c_condicion . "', tiene_nc_condicion = '" . $tiene_nc_condicion . "', tiene_na_condicion = '" . $tiene_na_condicion . "', prevencion_c_condicion = '" . $prevencion_c_condicion . "', prevencion_nc_condicion = '" . $prevencion_nc_condicion . "', prevencion_na_condicion = '" . $prevencion_na_condicion . "', supervisor_c_condicion = '" . $supervisor_c_condicion . "', supervisor_nc_condicion = '" . $supervidor_nc_condicion . "', supervisor_na_condicion = '" . $supervisor_na_condicion . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
					
			//Actualizar la info de cumplimiento
			$query = "UPDATE cumplimiento SET cumple_c_cumplimiento = '" . $cumple_c_cumplimiento . "', cumple_nc_cumplimiento = '" . $cumple_nc_cumplimiento . "', cumple_na_cumplimiento = '" . $cumple_na_cumplimiento . "', conoce_c_cumplimiento = '" . $conoce_c_cumplimiento . "', conoce_nc_cumplimiento = '" . $conoce_nc_cumplimiento . "', conoce_na_cumplimiento = '" . $conoce_na_cumplimiento . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
		
			//Actualizat la info de texto
			$query = "UPDATE texto SET actividades_texo = '" . $actividades_texto . "', hallazgos_texto = '" . $hallazgos_texto . "', calidad_texto = '" . $calidad_texto . "', topico_texto = '" . $topico_texto . "', incidentes_texto = '" . $incidentes_texto . "' WHERE id_reporte = " . $_SESSION['numero_reporte'] . " AND id_usuario = " . $_SESSION["id_usuario"];
			$resultado	= $nuevaConexion->ejecutarConsulta($query);
			...
			if($_FILES["foto_1"]["name"] != "" || $_FILES["foto_2"]["name"] != "" || $_FILES["foto_3"]["name"] != "" || $_FILES["foto_4"]["name"] != "" || $_FILES["foto_5"]["name"] != "" || $_FILES["foto_6"]["name"] != ""){
				...
				for($i=1;$i<=6;$i++){
					...
					if($_FILES[$nombre]['name'] != ""){
						...
						$extension = end($tmp);
						$nombre_imagen = "foto_" . $i . "_R_" . $numero_reporte . "." . $extension;
						...
						$query = "INSERT INTO imagenes (id_reporte, id_usuario, nombre_imagen, ruta_imagen, descripcion_imagen) VALUES ('" . $numero_reporte . "', " . $_SESSION["id_usuario"] . ", '" . $nombre_imagen . "', '" . $rutaImagen . "', '" . $descripcion_imagen . "')";
						$resultado	= $nuevaConexion->ejecutarConsulta($query);		
					}			
				}				
			}
			...
			if($_FILES['libro_obra']['name'] != ""){
				...
				$extension = end($tmp);
				$nombre_libro = "libro_" . "R" . $numero_reporte . "." . $extension;
				...
				$query = "INSERT INTO libro (id_reporte, id_usuario, nombre_libro, ruta_libro, descripcion_libro) VALUES ('" . $numero_reporte . "', " . $_SESSION["id_usuario"] . ", '" . $nombre_libro . "', '" . $rutaLibro . "', '" . $descripcion_libro . "')";
				$resultado	= $nuevaConexion->ejecutarConsulta($query);
			}
			...
			$descripcion = "Usuario ha actualizado el reporte diario N&deg;" . $numeroReporte . " el " . $nuevaConexion->enviarFechaHora();
			...
			//Redirigimos al informe_pdf.php
